(function(){const BASE_URL="https://chat.figolabs.ai";const DEFAULT_WIDGET_BUTTON={widgetButton:{backgroundColor:"#7351dd",textColor:"#fff",zIndex:10000,buttonText:"Ask AI",},};const ICON_SVG=`
  ...
  `;return btn;}function createCloseButton() {
  ...
  function initWidget(userConfig) {
		...
		config = { ...DEFAULT_WIDGET_BUTTON, ...userConfig, iframeUrl: `${BASE_URL}/chat/${userConfig.xClient}/${userConfig.assistantId}?${searchParams}` };
		destroyWidget();
  ...
  };window.FigoChatWidget={init:initWidget,shutdown,start,hide:hideIframe,destroy:destroyWidget,reinitialize:(newConfig)=>initWidget({...config,...newConfig}),onWidgetStateChange:(callback)=>{window.addEventListener("figoChatState",(event)=>callback(event.detail));},};})();